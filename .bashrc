...
__conda_setup="$('/localhome/rost_ph/miniforge3/bin/conda' 'shell.bash' 'hook' 2> /dev/null)"
if [ $? -eq 0 ]; then
    eval "$__conda_setup"
else
    if [ -f "/localhome/rost_ph/miniforge3/etc/profile.d/conda.sh" ]; then
        . "/localhome/rost_ph/miniforge3/etc/profile.d/conda.sh"
    else
        export PATH="/localhome/rost_ph/miniforge3/bin:$PATH"
    fi
fi
unset __conda_setup
# <<< conda initialize <<<
