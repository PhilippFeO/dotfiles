...
# │ VON MIR HINZUGEFÜGT │
# ╰─────────────────────╯
# in letzter Zeile steht auch noch etwas!
...
# "cpn" = "CoPy Nvim"; copy the nvim configuration into the shared folder of the VirtualBox.
#   Defined here because .bash_aliases is also used in my Virtual Machine
#   and this command is for copying some files "to" the Virtual Machine.
#   In the VM, there is a counterpart.
alias cpn='rm -fr ~/.VirtualBox/shared-folder/nvim \
    && cp -r ~/.config/nvim ~/.VirtualBox/shared-folder \
    && rm -r ~/.VirtualBox/shared-folder/.bash_aliases \
    && cp ~/.bash_aliases ~/.VirtualBox/shared-folder/.bash_aliases \
    && rm -r ~/.VirtualBox/shared-folder/.bashrc \
    && cp ~/.bashrc ~/.VirtualBox/shared-folder/.bashrc'
#    mv /.virtualbox/shared-folder/nvim/.git/ /dev/null \
